...
export type DelegateExecutionResult = {
  hash: `0x${string}` | null;
  error: string | null;
  /** Whether this transaction included a new delegation (Type 4) or used existing delegation (Type 2) */
  type: 'type4' | 'type2' | null;
};
...
export const walletExecuteWithDelegate = async ({
  ...
  try {
    ...
    // Type 4 = new delegation + swap, Type 2 = already delegated + swap
    const txType = wasAlreadyDelegated ? 'type2' : 'type4';

    return { hash: tx.txHash as `0x${string}`, error: null, type: txType };
  } catch (error) {
    return { hash: null, error: error instanceof Error ? error.message : String(error), type: null };
  }
};
