...
const BUTTON_CONTENT_OFFSET_X = 8;
const buttonContentTransitionConfig = buildTestSafeConfig({
  duration: 180,
  easing: easing.bezier.fade,
});

function buttonContentLayoutTransition(values: LayoutAnimationsValues) {
  'worklet';
  ...
  return {
    ...
    animations: {
      originX: withTiming(values.targetOriginX, buttonContentTransitionConfig),
      originY: values.targetOriginY,
      width: withTiming(values.targetWidth, buttonContentTransitionConfig),
      height: values.targetHeight,
    },
  };
}

function buttonContentEnterTransition(values: EntryAnimationsValues) {
  'worklet';
  ...
  return {
    initialValues: {
      ...
      originY: values.targetOriginY,
      transform: [{ translateX: BUTTON_CONTENT_OFFSET_X }],
    },
    animations: {
      opacity: withTiming(1, buttonContentTransitionConfig),
      originX: values.targetOriginX,
      originY: values.targetOriginY,
      transform: [{ translateX: withTiming(0, buttonContentTransitionConfig) }],
    },
  };
}

function buttonContentExitTransition(values: ExitAnimationsValues) {
  'worklet';
  ...
  return {
    ...
    animations: {
      opacity: withTiming(0, buttonContentTransitionConfig),
      originX: values.currentOriginX,
      originY: values.currentOriginY,
      transform: [{ translateX: withTiming(-BUTTON_CONTENT_OFFSET_X, buttonContentTransitionConfig) }],
    },
  };
}
...
export const layoutAnimations = {
  buttonContent: {
    layout: buttonContentLayoutTransition,
    entering: buttonContentEnterTransition,
    exiting: buttonContentExitTransition,
  },
} as const;
