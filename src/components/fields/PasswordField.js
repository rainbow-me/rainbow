...
const FieldAccessoryBadgeWrapper = styled(ShadowStack).attrs(({ theme: { colors, isDarkMode }, color }) => ({
...
const ShadowContainer = styled(ShadowStack).attrs(({ deviceWidth, theme: { colors, isDarkMode } }) => ({
...
const PasswordField = ({ isInvalid, isValid, password, returnKeyType = 'done', style, textContentType = 'password', ...props }, ref) => {
  ...
  return (
    <Container onPress={ios ? handleFocus : undefined}>
      <ShadowContainer deviceWidth={deviceWidth} isDarkMode={isDarkMode} style={style}>
        ...
        {isValid && <FieldAccessoryBadge color={colors.green} name="checkmarkCircled" />}
        {isInvalid && <FieldAccessoryBadge color={colors.orangeLight} name="warningCircled" />}
      </ShadowContainer>
    </Container>
  );
};
