...
export function CollectionCell({ collection }: { collection: MintableCollection }) {
  ...
  const isUnknownPrice = collection.mintStatus.price !== '0' && !collection.mintStatus.price;
  const amount = convertRawAmountToRoundedDecimal(collection.mintStatus.price, 18, 6);

  const isFree = !isUnknownPrice && !amount;
    ...
    <ButtonPressAnimation onPress={handlePress} style={{ width: NFT_IMAGE_SIZE }}>
      ...
      >
        ...
        <View style={{ width: NFT_IMAGE_SIZE - 16 }}>
          <Text color="label" size="11pt" weight="bold" numberOfLines={1} style={sx.mintPrice}>
            {isUnknownPrice ? i18n.t(i18n.l.minting.unknown) : isFree ? i18n.t(i18n.l.mints.mints_card.collection_cell.free) : amount}
          </Text>
        </View>
      </View>
