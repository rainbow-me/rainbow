import { isVersionGreaterThanOrEqualToAppVersion } from '../isTargetedVersionOrNewer';
...
describe('isVersionGreaterThanOrEqualToAppVersion', () => {
  afterEach(() => {
    jest.resetAllMocks();
  });
  ...
  it('should return true when current app version is newer than version to check', async () => {
    ...
    const result = await isVersionGreaterThanOrEqualToAppVersion('1.9.0');
  ...
  it('should handle versions with different number of parts', async () => {
    ...
    const result = await isVersionGreaterThanOrEqualToAppVersion('1.9.9');
    expect(result).toBe(true);
  });

  it('should return true when versions are equal but have different number of parts', async () => {
    (DeviceInfo.getVersion as jest.Mock).mockReturnValue('2.0.0');
    const result = await isVersionGreaterThanOrEqualToAppVersion('2.0');
    expect(result).toBe(true);
  });
  ...
  it('should return false when current app version is older with different number of parts', async () => {
    ...
    const result = await isVersionGreaterThanOrEqualToAppVersion('2.0.0');
    expect(result).toBe(false);
  });
});
