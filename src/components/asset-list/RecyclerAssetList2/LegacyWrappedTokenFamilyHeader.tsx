...
const TokenFamilyHeader = ({ childrenAmount, emoji, familyImage, isOpen, onPress, testID, title, theme }: TokenFamilyHeaderProps) => {
  const { colors } = theme;
    ...
    <ButtonPressAnimation onPress={handlePress} scaleTo={1.05} testID={testID}>
      ...
      >
        ...
        <View style={[sx.title, { paddingLeft: 10 }]}>
          <Text
            color={title === i18n.button.hidden() ? 'secondary60 (Deprecated)' : 'primary (Deprecated)'}
            numberOfLines={1}
        ...
        <View style={[sx.center, sx.amountContainer]}>
          ...
          <Animated.View style={imageAnimatedStyles}>
            <FastImage
              ...
              style={sx.chevron}
              tintColor={title === i18n.button.hidden() ? colors.blueGreyDark60 : colors.dark}
            />
          </Animated.View>
        </View>
      </View>
