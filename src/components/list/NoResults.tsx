...
export const NoResults = ({ onL2, type }: { onL2?: boolean; type: NoResultsType }) => {
  ...
  switch (type) {
    case NoResultsType.Discover:
      title = i18n.exchange.no_results.nothing_here();
      break;
    case NoResultsType.Swap:
      title = i18n.exchange.no_results.nothing_found();
      if (assetCount) {
        description = onL2 ? i18n.exchange.no_results.description_l2() : i18n.exchange.no_results.description();
      } else {
        description = i18n.exchange.no_results.description_no_assets({
          action: type,
        });
      }
      break;
    case NoResultsType.Send:
      title = i18n.exchange.no_results.nothing_to_send();
      description = i18n.exchange.no_results.description_no_assets({
        action: type,
      });
      break;
