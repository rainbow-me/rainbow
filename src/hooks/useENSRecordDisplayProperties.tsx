  ...
  const menuItems = useMemo(() => {
    return [
      allowEdit && type === 'record' && Object.values(ENS_RECORDS).includes(recordKey as ENS_RECORDS)
        ? {
            actionKey: 'edit',
            actionTitle: i18n.expanded_state.unique_expanded.edit(),
            icon: {
              iconType: 'SYSTEM',
      ...
      url
        ? {
            actionKey: 'open-url',
            actionTitle: i18n.expanded_state.unique_expanded.view_on_platform({ platform: isUrlValue ? 'Web' : label }),
      ...
      {
        actionKey: 'copy',
        actionTitle: i18n.expanded_state.unique_expanded.copy(),
