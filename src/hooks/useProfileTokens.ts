...
function useProfileTokenListMutation<TVariable>({
  ...
  return useMutation({
    mutationFn: async (variable: TVariable) => {
      // onMutate has already written the optimistic state to the cache; reuse it
      // verbatim so computeNext doesn't run twice for the same variable.
      const next = queryClient.getQueryData<string[]>(queryKey) ?? [];
      if (getIsReadOnlyWallet()) return next;
