...
export default function useAccountData() {
  ...
  const uniswapData = useSelector(
    ({ uniswap: { uniswapLiquidityTokenInfo } }) => ({
      uniswapLiquidityTokenInfo,
    })
  );
  const info = { ...accountSettings, ...uniswapData };

  return Object.assign(uniswapData, readableUniswapSelector(info));
}
