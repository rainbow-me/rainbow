...
const NEW_MARKET_ROW_LIGHT_GRADIENT_COLORS = ['rgba(131, 142, 153, 0.06)', 'rgba(131, 142, 153, 0)'] as const;
...
function NewMarketRow({ item }: { item: PerpMarketPlacementItem }) {
  ...
  const rowGradientColors = isDarkMode
    ? ([opacity(accentColor, 0.16), opacity(accentColor, 0)] as const)
    : NEW_MARKET_ROW_LIGHT_GRADIENT_COLORS;
