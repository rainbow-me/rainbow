...
import { NewTransaction, TransactionGasParamAmounts, TransactionStatus } from '@/entities';
import { ENSRegistrationRecords } from '../types/ensRegistrationTypes';
import { IS_TEST } from '@/env';
import { estimateENSTransactionGasLimit, formatRecordsForTransaction } from './ensHandler';
import { toHex } from '@/handlers/web3';
import { ENSRegistrationTransactionType, getENSExecutionDetails, REGISTRATION_MODES } from './ensHelpers';
...
import { saveCommitRegistrationParameters, updateTransactionRegistrationParameters } from './ensRegistrationRedux';
  ...
  try {
    ...
    switch (type) {
      case ENSRegistrationTransactionType.COMMIT:
        ...
        analytics.track(analytics.event.ensInitiatedRegistration, {
          category: 'profiles',
        });
        break;
      ...
      case ENSRegistrationTransactionType.REGISTER_WITH_CONFIG:
        ...
        analytics.track(analytics.event.ensCompletedRegistration, {
          category: 'profiles',
        });
        break;
      case ENSRegistrationTransactionType.RENEW:
        ...
        analytics.track(analytics.event.ensExtended, {
          category: 'profiles',
        });
        break;
      ...
      case ENSRegistrationTransactionType.RECLAIM:
        ...
        analytics.track(analytics.event.ensTransferredControl, {
          category: 'profiles',
        });
        break;
      case ENSRegistrationTransactionType.SET_NAME:
        ...
        analytics.track(analytics.event.ensSetPrimary, {
          category: 'profiles',
        });
    }
  } catch (e) {
...
const executeAction = async (
  ...
  try {
    ...
    analytics.track(analytics.event.ensRapFailed, {
      category: 'raps',
      failed_action: type,
      label: rapName,
    });
...
export const executeENSRap = async (
  ...
  analytics.track(analytics.event.ensRapStarted, {
    category: 'raps',
    label: rapName,
  });
  ...
  analytics.track(analytics.event.ensRapCompleted, {
    category: 'raps',
    label: rapName,
  });
