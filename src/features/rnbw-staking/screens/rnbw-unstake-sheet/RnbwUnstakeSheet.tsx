...
import { unstakeRnbw } from '../../../rnbw-staking/utils/unstakeRnbw';
...
import { useStakingBalance } from '../../../rnbw-staking/stores/derived/useStakingBalance';
import { UNSTAKE_PENALTY_PERCENTAGE } from '../../../rnbw-staking/constants';
...
const UnstakeContent = memo(function UnstakeContent() {
  ...
  const { tokenAmount, nativeCurrencyAmount, percentageOfExitFeeRecovered, stakeSessionPnl, isPositiveStakeSessionPnl } = useStableValue(
    () => useStakingBalance.getState()
  );
  ...
  const startUnstake = useCallback(async () => {
    ...
    try {
      ...
      // TODO: Remove raw error message display before production, allows easier internal testing
      ...
      <Box paddingHorizontal="16px" marginTop={{ custom: 48 }}>
        ...
        <Stack separator={<Separator color="separator" thickness={1} />}>
          ...
          <Box flexDirection="row" height={44} alignItems="center" justifyContent="space-between" width="full">
            ...
            <Text color={isPositiveStakeSessionPnl ? 'green' : 'red'} size="17pt" weight="bold">
              {`${stakeSessionPnl} ${RNBW_SYMBOL}`}
            </Text>
          </Box>
        </Stack>
      </Box>
