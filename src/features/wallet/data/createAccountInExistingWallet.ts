...
import { saveAddress, saveAllWallets, setSelectedWallet as setSelectedWalletInKeychain } from '@/features/wallet/data/walletKeychain';
import { type RainbowWallet } from '@/features/wallet/types';
import { generateAccount } from '@/model/wallet';
import { addAccountToWallet, getWallets } from '@/state/wallets/walletsStore';
...
export async function createAccountInExistingWallet({
  ...
  const updatedWallet = addAccountToWallet(id, {
  ...
  await Promise.all([saveAddress(account.address), setSelectedWalletInKeychain(updatedWallet), saveAllWallets(getWallets())]);
}
