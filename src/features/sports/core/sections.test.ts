...
describe('Sports sections', () => {
  it('groups retained games under the first matching Live group', () => {
    ...
    expect(
      getSportsSections({
        catalog,
        games,
        gameIds: ['basketball-match', 'tennis-match', 'scheduled'],
  ...
  it('caps each display section after sorting without changing the supplied membership', () => {
    const games = gamesById(
      ...Array.from({ length: 31 }, (_, index) => game(`live-${String(index).padStart(2, '0')}`)),
    ...
    expect(gameIds).toHaveLength(62);
  });
  ...
  it('orders browse games by promotion rank, kickoff, then ID without changing membership', () => {
    const games = gamesById(
      game('b'),
      game('a'),
...
describe('Sports directory counts', () => {
  it('counts retained browse and Search games once, including later status changes', () => {
    const games = gamesById(
      game('browse', { competitionIds: ['us-open', 'atp'] }),
      game('search-ended'),
      game('search-later'),
      game('lookup-only', { competitionIds: ['nba'] })
    );
    const browseIds = ['browse'];
    const searchIds = ['browse', 'search-ended', 'search-later'];

    games['search-ended'].status = Game_Status.STATUS_ENDED;
    games['search-later'].status = Game_Status.STATUS_SCHEDULED;
    games['search-later'].startsAt = new Date(2026, 9, 20).toISOString();
