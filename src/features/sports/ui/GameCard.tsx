...
function GameHeader({ gameId, scopeId }: { gameId: string; scopeId?: string }) {
  const competition = useSportsStore(state => {
    const ids = state.games[gameId]?.competitionIds;
...
function GameTime({ gameId }: { gameId: string }) {
  const { isDarkMode } = useColorMode();
  const live = useSportsStore(state => state.games[gameId]?.status === Game_Status.STATUS_LIVE);
  const clock = useSportsStore(state => state.games[gameId]?.clock);
  const period = useSportsStore(state => state.games[gameId]?.period);
  const startsAt = useSportsStore(state => state.games[gameId]?.startsAt);
  const status = useSportsStore(state => {
    const game = state.games[gameId];
    return game && (INTERRUPTION_LABELS[game.interruption] ?? STATUS_LABELS[game.status]);
  });
