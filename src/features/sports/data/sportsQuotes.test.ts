...
function game(id: string, firstToken = `${id}1`): Game {
...
function updateGames(...games: Game[]) {
  useSportsStore.setState(state => ({ games: { ...state.games, ...Object.fromEntries(games.map(game => [game.id, game])) } }));
}
...
it('delivers multiple visible offer changes once, without rebuilding subscriptions per game', async () => {
  ...
  expect(setSubscription).toHaveBeenLastCalledWith(owner, route, ['quote:a3', 'quote:a2', 'quote:b3', 'quote:b2']);
});
...
it('changes viewport demand without discarding rendered membership and drops dependencies on hidden games', async () => {
  ...
  await settle();
  expect(setSubscription).toHaveBeenLastCalledWith(owner, route, ['quote:b1', 'quote:b2']);
  ...
  sportsActions.setVisibleQuoteGames(owner, ['a', 'b']);
  await settle();
  expect(setSubscription).toHaveBeenLastCalledWith(owner, route, ['quote:a3', 'quote:a2', 'quote:b1', 'quote:b2']);
});
...
it('uses the lookup consumer as the only event viewport owner and follows only visible resolutions', async () => {
  ...
  await settle();
  expect(setSubscription).toHaveBeenLastCalledWith(owner, Routes.DISCOVER_SCREEN, ['quote:c1', 'quote:c2']);
...
it('keeps the observed event viewport and retained resolutions through deactivation and resumes current offers', async () => {
  ...
  await settle();
  expect(setSubscription).toHaveBeenCalledTimes(1);
  expect(setSubscription).toHaveBeenLastCalledWith(owner, Routes.DISCOVER_SCREEN, ['quote:a3', 'quote:a2']);
});
