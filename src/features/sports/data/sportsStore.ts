...
export type QuoteConsumer = { active: boolean; renderedGameIds: string[]; visibleGameIds: string[] };
...
export const useSportsViewStore = createBaseStore<SportsViewState>((set, get) => ({
  ...
  openScope: (host, scopeId) => {
    ...
    setBrowseRequest(host, { destination, query: null }, request.query === null ? navigationRoot : destination);
  },
  goBack: host => {
    const { request, navigationRoot } = get().hosts[host];
  ...
  refresh: async host => {
    if (!get().appActive || !get().hosts[host].visible) return;
...
function setBrowseRequest(host: SportsHost, request: BrowseRequest, navigationRoot?: SportsDestination): void {
  const { hosts, window } = useSportsViewStore.getState();
  const previousQuery = hosts[host].request.query;
  useSportsViewStore.setState({
    hosts: { ...hosts, [host]: { ...hosts[host], request, navigationRoot: navigationRoot ?? hosts[host].navigationRoot } },
  });
  ...
  useSportsStore.setState(state => {
    ...
    const key = getQueryKey({ request: { type: 'search', query: previousQuery }, window });
...
export type SportsState = {
  ...
  quoteTokens: Partial<Record<string, string[]>>;
...
function emptyData(): Omit<SportsState, 'clear'> {
  return { catalog: undefined, games: {}, quoteTokens: {}, results: {}, search: undefined, eventGames: {}, counts: {} };
}
...
function setSportsData({ data, params: { request, window }, queryKey, set }: SetDataParams<SportsResponse, SportsParams, SportsState>) {
  ...
  set(state => {
    ...
    let quoteTokens = state.quoteTokens;
    ...
    for (const incomingGame of data.games) {
      ...
      if (!sameQuoteTokens(previous, game)) {
        if (quoteTokens === state.quoteTokens) quoteTokens = { ...quoteTokens };
        quoteTokens[game.id] = gameQuoteTokens(game);
      }
    }
    ...
    const next = { catalog, games, quoteTokens, results, search, eventGames };
...
function sameQuoteTokens(previous: Game | undefined, next: Game): boolean {
  return (
    previous !== undefined &&
    previous.participants[0]?.winner?.tokenId === next.participants[0]?.winner?.tokenId &&
    previous.participants[1]?.winner?.tokenId === next.participants[1]?.winner?.tokenId &&
    previous.spread?.outcomes[0]?.tokenId === next.spread?.outcomes[0]?.tokenId &&
    previous.spread?.outcomes[1]?.tokenId === next.spread?.outcomes[1]?.tokenId &&
    previous.winner?.draw?.tokenId === next.winner?.draw?.tokenId
  );
}

function gameQuoteTokens(game: Game): string[] {
  const selections = [game.participants[0]?.winner, game.participants[1]?.winner, ...(game.spread?.outcomes ?? []), game.winner?.draw];
  return selections.flatMap(selection => (selection ? [selection.tokenId] : []));
}
...
function pruneGames(state: Pick<SportsState, 'games' | 'quoteTokens' | 'results' | 'search' | 'eventGames'>) {
  ...
  for (const id of Object.keys(games)) {
    ...
    delete games[id];
    delete quoteTokens[id];
  }
  return { games, quoteTokens };
}
