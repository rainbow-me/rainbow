...
function selectPredictionsPlacementItems(state: PlacementsState): PlacementItem<'polymarket'>[] {
  return state.getItemsBySource(PLACEMENT_IDS.DISCOVER_PREDICTIONS_CAROUSEL, 'polymarket');
}

function selectPredictionsEventIds(state: PlacementsState): string[] {
  return state.getRefIds(PLACEMENT_IDS.DISCOVER_PREDICTIONS_CAROUSEL, 'polymarket');
}

function hasPredictionsEventIds(state: PlacementsState): boolean {
  return state.hasRefIds(PLACEMENT_IDS.DISCOVER_PREDICTIONS_CAROUSEL, 'polymarket');
}
