import { PLACEMENT_IDS, PLACEMENT_SURFACES } from '@/features/placements/constants';
...
type HydratedPlacementItem = PlacementItem<'rainbow'> & {
  token: ResolvedToken;
};
...
const useTestPlacementStore = createPlacementStore({
  placementId: PLACEMENT_IDS.TOKENS,
  source: 'rainbow',
  ...
  select: ($, placementItems) => {
    ...
    for (const item of placementItems) {
      const token = tokensById[item.ref.id];
...
describe('createPlacementStore', () => {
  ...
  it('returns an empty result when the feature gate is off', () => {
    ...
    usePlacementsStore.setState({ placementsById: { [PLACEMENT_IDS.TOKENS]: createTokenPlacement() } });
  ...
  it('keeps resolver dependencies subscribed while the feature gate is off', async () => {
    ...
    const select = jest.fn(($, placementItems: PlacementItem<'rainbow'>[]) => {
  ...
  it('composes loading state from placements and the resolver', () => {
    ...
    expect(useTestPlacementStore.getState()).toMatchObject({
      isLoading: false,
      placement: expect.objectContaining({ id: PLACEMENT_IDS.TOKENS }),
    });
  });
...
function createTokenPlacement(): Placement {
  return {
    id: PLACEMENT_IDS.TOKENS,
    surfaces: [PLACEMENT_SURFACES.DISCOVER],
    enabled: true,
    version: 2,
    updatedAt: '2026-05-15T00:00:00.000Z',
    items: [{ order: 0, ref: { source: 'rainbow', type: 'token', id: 'tokenA' } }],
  };
}
