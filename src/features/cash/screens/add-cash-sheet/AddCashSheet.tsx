...
export const AddCashSheet = memo(function AddCashSheet() {
  ...
  // An explicit retry can reuse the order ID, but starts a new submission with a new submittedAt.
  const [submissionAtOpen] = useState(() => {
    const { status } = useCashBuyOrderStore.getState();
    return 'submittedAt' in status ? status.submittedAt : null;
  });
  const isRecoveredSubmission = submissionAtOpen !== null && submittedAt === submissionAtOpen;
  ...
  useEffect(() => {
    return () => {
      ...
      cashBuyOrderActions.suspendSubmission();
    };
  }, []);
    ...
    >
      <Box style={isKeypad ? styles.fullScreenContent : undefined}>
        {openGate ? (
          ...
          <PausedOrderContent onCheckAgain={handleCheckAgain} />
        ) : view === 'pending' ? (
          <PendingOrderContent variant={pendingVariant} />
        ) : view === 'keypad' ? (
          <KeypadAmountContent
            amount={amount}
            funding={funding}
