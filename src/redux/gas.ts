 ...
 */
export const updateGasFeeForSpeed = (speed: string, newMaxPriorityFeePerGas: string) => async (
...
export const gasPricesStartPolling = (network = Network.mainnet, flashbots = false) => async (
  dispatch: AppDispatch,
  getState: AppGetState
) => {
...
export const gasUpdateDefaultGasLimit = (defaultGasLimit = ethUnits.basic_tx) => (dispatch: AppDispatch) => {
  dispatch({
...
export const gasUpdateTxFee = (updatedGasLimit?: number, overrideGasOption?: string, l1GasFeeOptimism: BigNumber | null = null) => (
  dispatch: AppDispatch,
  getState: AppGetState
) =>
  withRunExclusive(async () => {
