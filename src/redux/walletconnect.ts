...
export const walletConnectRemovePendingRedirect =
  (type: WalletconnectResultType, scheme?: string | null) =>
...
export const setPendingRequest =
  (peerId: string, walletConnector: WalletConnect) =>
  (
...
export const getPendingRequest =
  (peerId: string) => (_: Dispatch, getState: AppGetState) => {
    const { pendingRequests } = getState().walletconnect;
  ...
  async (
    ...
    const matchingWalletConnectors = values(
      pickBy(
        walletConnectors,
        connector =>
          connector?.peerMeta?.url === dappUrl || !connector?.peerMeta
      )
    );
    try {
 ...
 */
export const saveWalletConnectUri =
  (uri: string) =>
  async (
