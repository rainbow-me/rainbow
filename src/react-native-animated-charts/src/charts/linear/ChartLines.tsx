...
function ChartLineFactory(orientation: 'horizontal' | 'vertical') {
  ...
  const ChartLine = React.memo(({ color = '#000000', thickness = 2, length, ...props }: ChartLineProps) => {
    ...
    const openingPositionHorizontalLineStyle = useAnimatedStyle(() => {
      ...
      return {
        ...
        transform: [
          {
            translateY: withTiming(firstPoint ? firstPoint?.y * height + 10 : 0),
          },
        ],
      };
    }, [currentPath]);
    return (
      <Animated.View
        ...
        style={[
          ...
          {
            height: isVertical ? length + FIX_CLIPPED_PATH_MAGIC_NUMBER : thickness,
            left: 0,
            position: 'absolute',
            top: 0,
