      ...
      <Box>
        <SheetActionButtonRow paddingBottom={5}>
          <HoldToAuthorizeButton
            ...
            hideInnerBorder
            label={insufficientEth ? i18n.profiles.confirm.insufficient_eth() : label}
            onLongPress={action}
...
export default function ENSConfirmRegisterSheet() {
  ...
  const stepLabel = useMemo(() => {
    if (mode === REGISTRATION_MODES.EDIT) return i18n.profiles.confirm.confirm_updates();
    if (mode === REGISTRATION_MODES.RENEW) return i18n.profiles.confirm.extend_registration();
    if (step === REGISTRATION_STEPS.COMMIT) return i18n.profiles.confirm.registration_details();
    if (step === REGISTRATION_STEPS.WAIT_COMMIT_CONFIRMATION) return i18n.profiles.confirm.requesting_register();
    if (step === REGISTRATION_STEPS.WAIT_ENS_COMMITMENT) return i18n.profiles.confirm.reserving_name();
    if (step === REGISTRATION_STEPS.REGISTER) return i18n.profiles.confirm.confirm_registration();
    if (step === REGISTRATION_STEPS.SET_NAME) return i18n.profiles.confirm.set_name_registration();
  }, [mode, step]);
    ...
    () => ({
      [REGISTRATION_STEPS.COMMIT]: (
        <TransactionActionRow
          ...
          label={i18n.profiles.confirm.hold_to_begin()}
          testID={step}
        />
      ),
      [REGISTRATION_STEPS.REGISTER]: (
        <TransactionActionRow
          ...
          isValidGas={Boolean(registrationCostsData?.isValidGas && registrationCostsData?.stepGasLimit)}
          label={i18n.profiles.confirm.hold_to_register()}
          testID={step}
        />
      ),
      [REGISTRATION_STEPS.RENEW]: (
        <TransactionActionRow
          ...
          isSufficientGas={Boolean(registrationCostsData?.isSufficientGasForRegistration && registrationCostsData?.isSufficientGasForStep)}
          isValidGas={Boolean(registrationCostsData?.isValidGas && registrationCostsData?.stepGasLimit)}
          label={i18n.profiles.confirm.hold_to_extend()}
          testID={step}
        />
      ),
      [REGISTRATION_STEPS.EDIT]: (
        <TransactionActionRow
          ...
          isSufficientGas={Boolean(registrationCostsData?.isSufficientGasForStep)}
          isValidGas={Boolean(registrationCostsData?.isValidGas && registrationCostsData?.stepGasLimit)}
          label={i18n.profiles.confirm.hold_to_confirm()}
          testID={step}
        />
      ),
