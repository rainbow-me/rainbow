        ...
        <Animated.View style={{ width: '100%', flexDirection: 'row', gap: 12, alignItems: 'center' }} entering={LAYOUT_ANIMATION}>
          ...
          {valueIconUrl ? (
            <Box flexDirection="row" alignItems="center" gap={4}>
              <FastImage source={{ uri: valueIconUrl }} style={{ width: 16, height: 16, borderRadius: 8 }} />
...
const CreatedBySection = memo(function CreatedBySection() {
  ...
  const creatorAddress = metadata?.rainbowTokenDetails?.onchainData?.creatorAddress;

  if (!creatorAddress) return null;

  return <CreatorInfoRow address={creatorAddress} icon="􀫸" label={i18n.t(i18n.l.expanded_state.sections.market_stats.created_by)} />;
});
...
export function AssetInfoList() {
  ...
  const hasCreatedBySection = useMemo(() => {
    return Boolean(metadata?.rainbowTokenDetails?.onchainData?.creatorAddress);
  }, [metadata]);
