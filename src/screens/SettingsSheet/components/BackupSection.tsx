        ...
        {backups.map(({ address, color, key, label: labelOrName, numAccounts, wallet }) => (
          <MenuItem
            ...
            rightComponent={
              <MenuItem.StatusIcon
                status={
                  wallet.backupType === WalletBackupTypes.cloud ? 'complete' : wallet.backedUp || wallet.imported ? 'incomplete' : 'warning'
                }
              />
            }
            size={60}
            titleComponent={<MenuItem.Title text={removeFirstEmojiFromString(labelOrName) || abbreviations.address(address, 4, 6) || ''} />}
          />
        ))}
      </Menu>
