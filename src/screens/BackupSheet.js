...
import { BackupCloudStep, BackupConfirmPasswordStep, BackupManualStep, BackupSheetSection } from '../components/backup';
  ...
  const renderStep = useCallback(() => {
    switch (step) {
      ...
      case WalletBackupStepTypes.cloud:
        return missingPassword ? <BackupConfirmPasswordStep /> : <BackupCloudStep />;
      ...
      default:
        return (
          <BackupSheetSection
            ...
            primaryLabel={`􀙶 ${lang.t('modal.back_up.default.button.cloud_platform', {
              cloudPlatformName: cloudPlatform,
            })}`}
            secondaryLabel={`🤓 ${lang.t('modal.back_up.default.button.manual')}`}
            titleText={lang.t('modal.back_up.default.title')}
            type="Default"
          />
        );
    }
  }, [goBack, missingPassword, onBackupNow, onIcloudBackup, onManualBackup, step]);
