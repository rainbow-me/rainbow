  ...
  if (SUPPORTED_NETWORKS.includes(nft.network)) {
    try {
      const res = await gretch<SuccessResponse, ErrorResponse>(
        `https://api${
          nft.network === Network.mainnet ? '' : `-${nft.network}`
        }.reservoir.tools/collections/v6?contract=${
      ...
      if (
        ...
        res?.data?.collections?.[0]?.floorAsk?.price?.currency?.symbol
      ) {
      ...
      logger.error(
        new RainbowError(`Error fetching NFT floor price from Reservoir: ${e}`)
      );
    }
  }
  return undefined;
