...
describe('Watch address flow', () => {
  ...
  it('Should show the "Add wallet modal" after tapping import with a valid address', async () => {
    ...
    await Helpers.typeText('import-sheet-input', 'james.eth', false);
  ...
  it('Should say "james.eth" in the Profile Screen header', async () => {
    if (device.getPlatform() === 'android') {
      // not working on android!
      // (app isn't idle and test times out)
      //   // await Helpers.checkIfElementByTextIsVisible('james.eth');
    } else {
