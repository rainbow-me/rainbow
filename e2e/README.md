...
To speedup getting the app into a specific state, we implement some commands. This is a deep link that we send to the app so it performs certain actions. The actions are implemented in `src/app/navigation/TestDeeplinkHandler.tsx`, and can be launched by using the following yaml.
